...
if ismapping(d)
  ...
  W = prmapping(mfilename,'trained',{Q, pars{2},pars{3},L,pars{5},pars{6}},[],m,length(J));
  W = setname(W,'PE embedding');
  return
end
...
if pzero == 0,
  W = prmapping(mfilename,'trained',{Q,mean(+d,2)',pzero,L,sig,prec},[],m,sum(sig));
else
  W = prmapping(mfilename,'trained',{Q,+d(:,pzero)',pzero,L,sig,prec},[],m,sum(sig));
end
W = setname(W,'PE embedding');
return
